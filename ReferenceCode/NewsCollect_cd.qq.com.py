...
def _GetResponse(url):
    ...
    if response.info().get('Content-Encoding') == 'gzip':
        buf = StringIO( url_read)
        f = gzip.GzipFile(fileobj=buf)
        url_read = f.read()
    try:
        url_read = url_read.decode('GB2312').encode('utf8')    # 将网页从GBK解码为utf-8
    except:
        try:
            url_read = url_read.encode('utf8')    # 将网页从GBK解码为utf-8
        except:
            pass
    return url_read
...
print json.dumps(news_nopuntitles, encoding="UTF-8", ensure_ascii=False)